...
**Segmenter.** `derive_mode = "segmenter"` swaps the model-driven batch step for a deterministic one. It clusters spans by which typed anchors keep recurring (a ticket key, a branch, a document, a place), scores each cluster against every task's evidence profile, uses hysteresis so a momentary glance folds back into the surrounding work, and proposes each cluster that matches nothing as new work: its time sits on the project's other work, the model names the card, and confirming it creates the task. The default is the model path.
