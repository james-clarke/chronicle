...
| 24 | **Live feed (parked concept):** Unassigned as the front door — block states (fresh / provisional / proposed / assigned / unmatched / ejected), deterministic pre-pass (branch → ticket, repo signal, corrections FTS) writing provisional intervals before the batch derive, proposal cards for unmatched clusters, eject with negative corrections — see `m24-live-feed-plan.md` (open questions pending) | a block on a ticketed branch shows under its task within one tick; an ejected block never returns to that task |
